--SQL for extracting the customer Demension
...
--SQL for extracting the store Demension
...
--SQL for extracting the Product Demension
--Product
insert into ruthergg_ZAGIMORE_DS.ProductDimension(ProductID, ProductName, ProductPrice, Vendorid, Vendorname, CategoryID, CategoryName, ProductType)
...
--Rental Product
insert into  ruthergg_ZAGIMORE_DS.ProductDimension(ProductID, ProductName, ProductDailyRentalPrice, ProductWeeklyRentalPrice, Vendorid, Vendorname, CategoryID, CategoryName, ProductType)
...
--Fact Table 
--Step 1 Fact Extraction into intermiate fact table (Sales)
...
--Step 2 Populating fact tbale with adta form interdiate fact table
insert into Revenue_and_Units_Dimension(UnitsSold, RevenueGenerated, RevenueType, TransactionID, CustomerKey, StoreKey, ProductKey, CalendarKey)
...
where i.CustomerID = cud.CustomerID and i.storeid = sd.storeid and i.productid = pd.productid and pd.ProductType = "Sales" and i.FullDate = cad.CalendarDate
-- INtermediate step
Drop Table intermediate_facttable
--Step 3 Fact Extraction into intermiate fact table (Rental)
Create Table intermediate_facttable as 
select 0 as UnitsSold, sv.duration * p.productpricedaily as RevenueGenerated, sv.tid as TransactionID, "Rental,Daily" as RevenueType,
 p.productid as ProductID, st.storeid as StoreID, st.customerid as CustomerID, st.tdate as FullDate
from ruthergg_ZAGIMORE.rentvia sv, ruthergg_ZAGIMORE.rentalProducts p, ruthergg_ZAGIMORE.rentaltransaction st
where p.productid = sv.productid and st.tid = sv.tid and sv.rentaltype = "D"
-- 
...
from ruthergg_ZAGIMORE.rentvia sv, ruthergg_ZAGIMORE.rentalProducts p, ruthergg_ZAGIMORE.rentaltransaction st
where p.productid = sv.productid and st.tid = sv.tid and sv.rentaltype = "W"
--Step 4 
insert into Revenue_and_Units_Dimension(UnitsSold, RevenueGenerated, RevenueType, TransactionID, CustomerKey, StoreKey, ProductKey, CalendarKey)
select i.UnitsSold, i.RevenueGenerated, i.RevenueType, i.TransactionID, cud.CustomerKey, sd.StoreKey, pd.ProductKey, cad.CalendarKey
From intermediate_facttable i, CustomerDimension cud, StoreDimension sd, ProductDimension pd, CalendarDimension cad
where i.CustomerID = cud.CustomerID and i.storeid = sd.storeid and i.productid = pd.productid and pd.ProductType = "Rental" and i.FullDate = cad.CalendarDate
-- Data Warehouse Load
-- Customer
...
from ruthergg_ZAGIMORE_DS.StoreDimension
-- Product
...
from ruthergg_ZAGIMORE_DS.ProductDimension
-- Calendar
insert into ruthergg_ZAGIMORE_DW.CalendarDimension(CalendarKey,CalendarDate,CalendarMonth,CalendarYear, DayofWeek)
select CalendarKey,CalendarDate,CalendarMonth,CalendarYear,DayofWeek
from ruthergg_ZAGIMORE_DS.CalendarDimension
-- Fact Table
